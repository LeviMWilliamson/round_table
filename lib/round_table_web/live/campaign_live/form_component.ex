defmodule RoundTableWeb.CampaignLive.FormComponent do
  use RoundTableWeb, :live_component

  alias RoundTable.Games

  @impl true
  def update(%{campaign: campaign} = assigns, socket) do
    changeset = Games.change_campaign(campaign)
  ...
  def handle_event("validate", %{"campaign" => campaign_params}, socket) do
    changeset =
      ...
      |> Games.change_campaign(campaign_params)
  ...
  defp save_campaign(socket, :edit, campaign_params) do
    case Games.update_campaign(socket.assigns.campaign, campaign_params) do
      {:ok, _campaign} ->
        {:noreply,
         socket
  ...
  defp save_campaign(socket, :new, campaign_params) do
    case Games.create_campaign(campaign_params) do
      {:ok, _campaign} ->
        {:noreply,
         socket
